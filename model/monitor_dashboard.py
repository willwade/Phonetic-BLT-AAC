        ...
        while True:
            if metrics_file.exists():
                ...
                if current_size > last_size:
                    ...
                    if latest:
                        m = latest[0]
                        print(f"\r[{m['epoch']:02d}|{m['step']:04d}] "
                              f"Loss: {m['loss']:.4f} | "
                              f"PPL: {m['perplexity']:.2f} | "
                              f"GPU: {m['gpu_memory_gb']:.1f}GB | "
                              f"LR: {m['learning_rate']:.6f}    ", end="", flush=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--interval", type=int, default=10, help="Update interval for watch mode (seconds)")
